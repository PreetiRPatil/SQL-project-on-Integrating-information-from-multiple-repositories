...
/***********  Table Type ****************/
create or replace type tab_typ_varchar as table of varchar2(50);
/**************  Function to split the global query**************************/
create or replace function regex_substr (str in varchar2) return tab_typ_varchar as
  ret  tab_typ_varchar := tab_typ_varchar();
  i    number := 1;
  value_store varchar2(50);
begin
  loop   
    value_store := regexp_substr(str, '([^[:blank:]]+)', 1,  i);
    exit when value_store is null;
    ret.extend;
    ret(i) := value_store;
    i := i+1;
  end loop;
...
/***********  Stored Procedure which takes the global query and temporary table name as  input strings and generate sub queries & temporary tables that contain the results for each submitted subquery to each local database  & gives final output****************/
set serveroutput on;
show errors;
